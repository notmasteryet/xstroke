...
void *grid_feature_data_alloc(rec_engine_t *engine, const char *feature_data_str)
{
...
{
    return strdup((const char *) stroke->classifications[engine->num]);
}
...
int grid_set_option(rec_engine_t *engine, const char *name, const char *value)
{
    grid_priv_t *priv;
