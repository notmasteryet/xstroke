...
int rec_init(rec_t *rec, int width, int height)
{
    int i, err;
    char *log_file;
    ...
    rec->rotation = ARGS_DEFAULT_ROTATION;
    rec->rotation_limit = ARGS_DEFAULT_ROTATION_LIMIT;
...
rec_engine_t *rec_get_engine(rec_t *rec, const char *engine_name)
{
    int i;
...
rec_mode_t *rec_get_mode(rec_t *rec, const char *mode_name)
{
    int i;
    rec_mode_t *new_mode;
...
#include "./lex_regs.inc"
#include "./init_rec.inc"
